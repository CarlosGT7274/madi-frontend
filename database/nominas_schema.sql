-- =====================================================
-- SCRIPT SQL - MÓDULO DE NÓMINAS
-- Proyecto: MADI - Sistema de Gestión
-- =====================================================

-- =====================================================
-- 1. MODIFICAR TABLA USUARIOS (Agregar precio_hora)
-- =====================================================

-- Agregar columna precio_hora a la tabla usuarios existente
ALTER TABLE usuarios ADD COLUMN precio_hora DECIMAL(10,2) DEFAULT 150.00 COMMENT 'Precio por hora del empleado para cálculo de nóminas';

-- Crear índice para optimizar consultas
CREATE INDEX idx_usuarios_precio_hora ON usuarios(precio_hora);

-- Actualizar precios por rol (valores sugeridos)
UPDATE usuarios u
INNER JOIN roles r ON u.rol_id = r.id
SET u.precio_hora = 200.00
WHERE r.nombre = 'Supervisor';

UPDATE usuarios u
INNER JOIN roles r ON u.rol_id = r.id
SET u.precio_hora = 150.00
WHERE r.nombre = 'Obrero' OR r.nombre = 'Empleado';

UPDATE usuarios u
INNER JOIN roles r ON u.rol_id = r.id
SET u.precio_hora = 250.00
WHERE r.nombre = 'Ingeniero';

UPDATE usuarios u
INNER JOIN roles r ON u.rol_id = r.id
SET u.precio_hora = 180.00
WHERE r.nombre = 'Técnico';

-- =====================================================
-- 2. MODIFICAR TABLA COTIZACIONES (Agregar mano de obra)
-- =====================================================

-- Agregar columna para precio de mano de obra
ALTER TABLE cotizaciones ADD COLUMN precio_mano_obra DECIMAL(12,2) DEFAULT 0.00 COMMENT 'Costo total de mano de obra calculado desde planeaciones';
ALTER TABLE cotizaciones ADD COLUMN horas_mano_obra DECIMAL(10,2) DEFAULT 0.00 COMMENT 'Total de horas trabajadas en la cotización';

-- Crear índice
CREATE INDEX idx_cotizaciones_mano_obra ON cotizaciones(precio_mano_obra);
...
CREATE TABLE nomina_detalle_planeaciones (
  id INT AUTO_INCREMENT PRIMARY KEY,
...
-- Trigger: Cuando se actualiza horas_trabajadas en asignaciones
-- Auto-incrementa precio_mano_obra en la cotización relacionada
